        ...
        private void DragSource_PreviewMouseMove(object sender, MouseEventArgs e) {
            if (draggedData != null) {
                ...
                if (Utilities.IsMovementBigEnough(initialMousePosition, e.GetPosition(topWindow))) {
                    initialMouseOffset = initialMousePosition -
                                         sourceItemContainer.TranslatePoint(new Point(0, 0), topWindow);
                    ...
                    try {
                        DragDropEffects effects = DragDrop.DoDragDrop((DependencyObject) sender, data,
                                                                      DragDropEffects.Move);
                    }catch(InvalidOperationException) {
                       // throw new InvalidOperationException();
                        MessageBox.Show("nieudana operacja przeniesienia");
                    }
                ...
                if (targetItemsControlCount > 0) {
                    ...
                    if (targetItemContainer != null) {
                        ...
                        isInFirstHalf = Utilities.IsInFirstHalf(targetItemContainer, positionRelativeToItemContainer,
                                                                hasVerticalOrientation);
                        insertionIndex =
                            targetItemsControl.ItemContainerGenerator.IndexFromContainer(targetItemContainer);
                        ...
                        targetItemContainer =
                            targetItemsControl.ItemContainerGenerator.ContainerFromIndex(targetItemsControlCount - 1) as
                            FrameworkElement;
                        isInFirstHalf = false;
